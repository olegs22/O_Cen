...
events, Ocen_exp, Ocen_psf, background = np.loadtxt(path + 'OC_no_events_15_bins_0.15_degree.txt',usecols=(1,4,5,6),unpack=True)#15 bin data
...
Ps_exp2, Ps_psf2 = np.loadtxt(path + 'source2_J1328_0.15_degree.txt',usecols=(2,3),unpack=True)#15 bin data
...
def priors1(pars,plist):
    Gamma,log_E,log_N,log_Nn2,alpha2  = pars
   ...
   # mu_n1 = 3.8873e-12
    ...
    if plist[0]<Gamma<plist[1] and plist[2]<log_E<plist[3] and plist[4]<log_N<plist[5] and\
       plist[6]<log_Nn2<plist[7] and plist[8]<alpha2<plist[9]:
